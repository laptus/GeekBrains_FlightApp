#import "ApiManager.h"
#define API_TOKEN @"3cab31e7756b1fa58826b3ac8e9073eb"
...
////#define API_URL_MAP_PRICE @"http://map.aviasales.ru/supported_directions.json?origin_iata="
...
+(instancetype) sharedInstance{
    static ApiManager* ipManager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        ipManager = [[ApiManager alloc]init];
    });
    return ipManager;
}
...
-(void) cityForCurrentIP:(void(^)(City* city))completion{
    [self IPAddressWithCompletion:^(NSString *ipAddress) {
        [self load:[NSString stringWithFormat:@"%@%@", API_URL_CITY_FROM_IP, ipAddress] withCompletion:^(id  _Nullable result) {
            ...
            NSString* data = [json valueForKey:@"iata"];
            if (data){
                City* city = [[DataManager sharedInstance] cityForIata:data];
                if (city){
                    dispatch_async(dispatch_get_main_queue(), ^{
                        completion(city);
                    });
...
//    [sessionConfiguration setHTTPAdditionalHeaders:@{ @"X-Auth-Token" : API_TOKEN }];
    //[[NSURLSession sharedSession].configuration setHTTPAdditionalHeaders:@{ @"X-Auth-Token" : API_TOKEN }];
...
NSString* SearchRequestQuery(SearchRequest request){
    ...
    if (request.departTime && request.returnDate){
        NSDateFormatter* dateFormatter = [[NSDateFormatter alloc] init];
        dateFormatter.dateFormat = @"yyyy-MM";
        result = [NSString stringWithFormat:@"%@&depart_date=%@&return_Date=%@", result,[dateFormatter stringFromDate:request.departTime], [
